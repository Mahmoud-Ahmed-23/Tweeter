...
namespace Tweeter.Core.Application.Services.Identity.Authentication
{
	internal class AuthenticationService : IAuthenticationService
	{
		...
		public async Task<Result<ReturnUserDto>> Login(string email, string password)
		{
			...
			if (user is null)
				return Result<ReturnUserDto>.Fail("Un authorized!", ErrorType.Unauthorized);

			var result = await _signInManager.CheckPasswordSignInAsync(user, password, true);

			if (!user.EmailConfirmed)
				return Result<ReturnUserDto>.Fail("Email is not confirmed", ErrorType.Unauthorized);

			if (result.IsLockedOut)
				return Result<ReturnUserDto>.Fail("User is locked out", ErrorType.Unauthorized);
